...
bus_neighbour=Dict()
for (bus_id,bus) in BE_grid_energy_island["bus"]
    neighbour=[]
    for (br,branch) in BE_grid_energy_island["branch"]
        # Connect from
...
for (bus_id,bus) in BE_grid_energy_island["bus"]
    ...
    push!(BE_grid_energy_island["bus"][bus_id],"visited_node_all_bfs"=>visited_node)
end

for (b_id,b) in BE_grid_energy_island["bus"]
